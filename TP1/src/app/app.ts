...
export class AppComponent {
  ...
  async searchAlbums() {
    ...
    if (!artist) {
      this.errorMsg = 'Veuillez entrer un nom d’artiste.';
      return;
    }
    this.loadingAlbums = true;

    const url = `${environment.LASTFM_BASE}?method=artist.gettopalbums&artist=${encodeURIComponent(artist)}&api_key=${environment.LASTFM_API_KEY}&format=json&limit=24`;
    ...
    try {
      ...
      if (this.albums.length === 0) {
        this.errorMsg = 'Aucun album trouvé pour cet artiste.';
      }
    } catch (e: any) {
      this.errorMsg = 'Impossible de récupérer les albums. Vérifiez le nom ou réessayez.';
    } finally {
  ...
  async loadTracksForAlbum(album: Album) {
    ...
    const url = `${environment.LASTFM_BASE}?method=album.getinfo&${params}&api_key=${environment.LASTFM_API_KEY}&format=json`;
    ...
    try {
      ...
      if (this.tracks.length === 0) {
        this.errorMsg = 'Aucune chanson trouvée pour cet album.';
      }
    } catch (e: any) {
